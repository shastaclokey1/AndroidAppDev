...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes()
    {
        ...
        try {
            ...
            JSONArray earthquakeFeatures = earthquakeJson.getJSONArray("features");

            //set up a json object to store the data from each element in the earthquake features json array
            JSONObject currentEarthquake;

            //set up the current value variables to be stored in the Earthquake arraylist
            String currentMagnitude, currentLocation, currentDate, currentUrl;

            //Loop through each feature in the array
            for (int i = 0; i < earthquakeFeatures.length(); i++)
            {
                //Get earthquake JSONObject at position i and Get “properties” JSONObject
                currentEarthquake = earthquakeFeatures.getJSONObject(i).getJSONObject("properties");

                //Extract “mag” for magnitude, Extract “place” for location, Extract “time” for time, and extract the url for loading usgs web page
                currentMagnitude = currentEarthquake.getString("mag");
                currentLocation = currentEarthquake.getString("place");
                currentDate = currentEarthquake.getString("time");
                currentUrl = currentEarthquake.getString("url");


                //Create Earthquake java object from magnitude, location, and time and Add earthquake to list of earthquakes
                earthquakes.add(new Earthquake(currentMagnitude, currentLocation, currentDate, currentUrl));
            }
